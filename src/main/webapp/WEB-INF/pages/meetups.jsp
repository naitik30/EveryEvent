...
<head>
  ...
  .jumbotron {
    position: relative;
    background: #000 url("https://lonelyplanetimages.imgix.net/a/g/hi/t/9cf024dfd5c0bcb2b17f4785340145ea-san-francisco.jpg") center center;
    width: 100%;
    height: 100%;
...
<nav class="navbar navbar-default navbar-fixed-top">
 ...
 <div class="jumbotron text-center">
  ...
  <form class="form-inline" action="searchmeetupsReq" method="POST">
  
   	  <select class="topic1 form-control" id="topic" name="topic"  style="border-radius: 4px;width:10%">
					...
					<option>Moms &amp; Dads</option>
					<option>Pets</option>
					<option>Photography</option>
					<option>Religion @amp Beliefs,</option>
	  ...
	  <input class="istopic" type="checkbox" name="istopic" value="1" onchange="valueChanged()"/>
		<script type="text/javascript">
		function valueChanged()
		{
		    if($('.istopic').is(":checked"))   
		        $(".topic1").hide();
		    else
		        $(".topic1").show();
		}
	  ...
	  <input id="city" type="text" name="city" size=20 class="form-control" style="border-radius: 4px;" placeholder="Search cities..." autocomplete="on" />
       ...
       <input id="state" type="text" name="state" size=5 class="form-control" style="border-radius: 4px;" placeholder="CA" autocomplete="on" />
      	 <script>
            $(function() {
                function displayResult(item) {
                    $('.alert').show().html('You selected <strong>' + item.value + '</strong>: <strong>' + item.text + '</strong>');
...
<div id="tour" class="bg-1">
  <div class="container">
    <h3 class="text-center">Meetups For Location :</h3>
    ...
    <c:forEach var="items" items="${listOfEvents}">
      ...
      <div class="col-sm-4">
        ...
        <div class="thumbnail">
          <img src="${items.logoURL}" alt="No Image Available" style=" height: 230Px;" width="400" height="300">
          <p><strong>${items.name}</strong></p>
          <p>${items.venue.city }</p>
       		       <form action="${pageContext.request.contextPath}/searchmeetupsByIDReq?eventID=${items.event_id}" method="Post">
		                          		 	<input type="submit" class="btn" id="submit" value="See Event" > 
		    	 </form>
	     </div>
      </div>
    ...
    <c:if test="${count%3==0}">
     ...
     <c:set var="count" value="${count+1}" scope="page"/>
        ...
        <script type="text/javascript">
        $('#showmeetuppage').on('click', function (e) {
