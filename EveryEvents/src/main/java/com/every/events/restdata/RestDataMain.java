	...
	public static ArrayList<Events> DecodeJSON(String events){
		...
		try{
			...
			while(i.hasNext()){
				...
				{
					e.setEvent_id(Long.parseLong(event.get("id").toString()));
				}
				...
				{
					e.setName(event.get("status").toString());
				}
				// time
				...
				if(event.containsKey("fee"))
				{
					...
					{
						e.setAmount(Long.parseLong(fee.get("amount").toString()));
					}
